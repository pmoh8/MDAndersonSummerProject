...
>>>>>>> ca6d605902d5c43b29e9b9eb362b1e8b3fddf956
                            br(),
...
server <- function(input, output) {
  ...
  #END TEST VALUES
    ...
    # DNAcopy_options <- unique(inputdata_x[,"SYMBOL"])
  ...
  #######################################
    output$cancer_selector <- renderUI({
      selectInput("variable", NULL, cancer_options, multiple = TRUE)
    })
    output$gene_x_selector <- renderUI({
      selectInput("gene_x", NULL, DNAcopy_options())
      # selectInput("gene_x", NULL, as.list(DNAcopy_options))
    })
    
    y_options <- unique(inputdata_y[,"SYMBOL"])
  ...
  ##################################
    ...
    output$plot<- renderPlot({ 
      ...
      if(x_sel != " " && y_sel != " "){
        ...
        if(!is.null(input$plot_zoom_click) || (is.null(input$plot_zoom_click) && !is.null(pnt_zoom))){
      ...
      #If the user has selected a point, highlight it on the scatterplot
        ...
        if(is.null(input$plot_click) && is.null (pnt) ){
          defaultplot
        }
        else{
    ...
    output$plot_zoom<- renderPlot({ 
      ...
      #Allow for point that was selected on hte "zoomed-out" plot to appear in the "zoomed-in" plot
       ...
       #If the user has selected a point on the "zoomed-in" plot, highlight it on the scatterplot
        #If there is no mouse click, continue printing out the same graph as previously 
          #(Prevent the highlighted point from flickering in and out of existence between clicks)
        if(is.null(input$plot_zoom_click) && is.null (pnt_zoom) ){
          zoomplot
        }
        else{
  ...
  #################################
    ...
    printPoint <- function(point){
      if(is.data.frame(point) && nrow(point)!=0) {
        pointDetail <- data.frame(x = point$x, y = point$y)
        names(pointDetail)<-c(x_sel,y_sel)
        row.names(pointDetail)<-c("DNA Copy #")
        return (pointDetail)
      }
    }
