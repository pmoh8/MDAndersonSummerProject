...
server <- function(input, output) {
  ...
  ##################################
    ...
    output$plot_zoom<- renderPlot({ 
      ...
      zoomplot<<-ggplot(plotdata$zoomdata, aes(x,y)) + geom_point(alpha=transp, color=colDefault) + geom_point(data = points$pnt, aes(x,y), color=colRed, size = ptMed) 
      
      #plot the user-click highlighted points
      zoomplot <- plotPointOnClick(zoomplot, input$plot_click, points$pnt, colOrn, ptMed, updatePoint = FALSE)
      zoomplot <- plotPointOnClick (zoomplot, input$plot_zoom_click, points$pnt_zoom, colRed, ptBig, updatePoint = TRUE)
      
      #update the stored point of interest if needed
      if(!(is.null(input$plot_zoom_click) && is.null(points$pnt_zoom) ) && !is.null(input$plot_zoom_click)){
        points$pnt_zoom <<- nearPoints(defaultplotdata, input$plot_zoom_click, maxpoints = 1)
      }
